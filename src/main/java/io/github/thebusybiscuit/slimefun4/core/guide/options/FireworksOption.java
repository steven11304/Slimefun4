...
class FireworksOption implements SlimefunGuideOption<Boolean> {
    ...
    public Optional<ItemStack> getDisplayItem(Player p, ItemStack guide) {
        if (SlimefunPlugin.getRegistry().isResearchFireworkEnabled()) {
            ...
            ItemStack item = new CustomItem(Material.FIREWORK_ROCKET, "&b煙花: &" + (enabled ? "a啟用" : "4禁用"), "", "&7You can now toggle whether you", "&7will be presented with a big firework", "&7upon researching an item.", "", "&7\u21E8 &eClick to " + (enabled ? "disable" : "enable") + " your fireworks");
